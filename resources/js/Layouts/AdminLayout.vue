<template>
    <div
        class="flex h-screen bg-[#f8fafc] dark:bg-slate-950 overflow-hidden transition-colors duration-200"
    >
        <!-- ─────────────────── SIDEBAR ─────────────────── -->
        <aside
            class="w-[260px] bg-white dark:bg-slate-900 border-r border-gray-100 dark:border-slate-700/50 flex flex-col fixed inset-y-0 left-0 z-20 transition-colors duration-200"
        >
            <!-- Logo -->
            <div
                class="h-16 px-5 flex items-center gap-3 border-b border-gray-100 dark:border-slate-700/50 flex-shrink-0"
            >
                <div
                    class="w-8 h-8 rounded-xl bg-blue-600 flex items-center justify-center shadow-sm"
                >
            ...
            <nav class="flex-1 px-3 py-4 overflow-y-auto">
                <template
                    v-for="(grupo, gIdx) in navGroups"
                    :key="grupo.label || `g-${gIdx}`"
                >
                    <p
                        v-if="grupo.label"
                    ...
                    <div
                        v-else-if="gIdx > 0"
                        class="my-3 mx-3 border-t border-gray-100 dark:border-slate-700/50"
                    ></div>
            ...
            <!-- User + Logout -->
            <div
                class="flex-shrink-0 border-t border-gray-100 dark:border-slate-700/50 p-3 space-y-1"
            >
                <div class="flex items-center gap-3 px-2 py-2">
                    ...
                    <div class="flex-1 min-w-0">
                        <p
                            class="text-[13px] font-semibold text-gray-800 dark:text-slate-100 truncate leading-tight"
                        >
                            {{ user?.name }}
                        </p>
                        <p
                            class="text-[11px] text-gray-400 dark:text-slate-500 leading-tight mt-0.5"
                        >
                            {{ roleLabel }}
                        </p>
                    </div>
                ...
                <Link
                    ...
                    method="post"
                    as="button"
                    class="flex items-center gap-2.5 w-full px-3 py-2 rounded-xl text-[13px] font-medium text-gray-400 dark:text-slate-500 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                >
        ...
        <div class="flex-1 ml-[260px] flex flex-col min-h-0">
            <!-- Top bar -->
            <header
                class="h-14 bg-white dark:bg-slate-900 border-b border-gray-100 dark:border-slate-700/50 flex items-center px-6 gap-3 flex-shrink-0 z-10 transition-colors duration-200"
            >
                <!-- Breadcrumb -->
                <div class="flex items-center gap-2 text-sm min-w-0">
                    ...
                    <template v-if="pageTitle">
                        <span
                            class="text-gray-200 dark:text-slate-700 flex-shrink-0"
                            >/</span
                        >
                        <span
                            class="font-semibold text-gray-700 dark:text-slate-200 truncate"
                            >{{ pageTitle }}</span
                        >
                    </template>
                </div>
                ...
                <div class="ml-auto flex items-center gap-1">
                    <button
                        @click="toggleTheme"
                        class="w-9 h-9 rounded-xl flex items-center justify-center text-gray-400 dark:text-slate-400 hover:text-gray-700 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-slate-700 transition-colors"
